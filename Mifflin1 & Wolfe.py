...
"""
the final result is [ 2.19908259e-01 -1.31942515e-04],the funtion value is[-0.21990826  3.29862494],the iteration number is 8555
Elapsed time: 13.186151266098022 seconds
...
def f2(x):
    if x[0]>= np.abs(x[1]):
        res = 5*np.sqrt(9*(x[0]**2)+16*(x[1]**2))
    elif 0<x[0]<=np.abs(x[1]):
        res = 9*x[0] + 16*np.abs(x[1])
    elif x[0]<=0:
        res = 9*x[0] + 16*np.abs(x[1]) - x[0]**9
    return res
...
x = np.array([-0.5,-0.5])  # Define x as a float64 array
