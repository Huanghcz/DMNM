...
"""
the final result is [0.9994439  0.99944391],the funtion value is[ 2.002225   -1.00111157],the iteration number is 1416
Elapsed time: 2.6113462448120117 seconds
"""
...
#
# def f2(x):
#     res = np.abs(x[1]- 10*np.abs(x[0]) + 0.5*x[1])
#     return res
...
def main(x, var_e, delta, c, t0):
    results = {'Iteration': [], 'X': [], 'Function Values': []}
    ...
    while True:
        ...
        iteration_results = {
            'Iteration': j,
            'X': x.tolist(),
            'Function1 Values': f1(x).tolist(),
            'Function2 Values': f2(x).tolist()
        }
        results['Iteration'].append(iteration_results)
        print(f'Iteration {j}:')
        print(f'Current x: {x}')
        print(f'Function values at x: {f(x)}')
        if np.linalg.norm(v) <= delta:
            break
        s = solver(f, t0, v, x)
    ...
    # 将结果保存为Excel文件
    df = pd.DataFrame(results['Iteration'])
    df.to_excel('iteration_results.xlsx', index=False)
    selected_data = df.iloc[::200, :]
    selected_data.to_excel('iteration_results2.xlsx', index=False)


    return x, j, elapsed_time

x = np.array([2.0, 2.0])  # Define x as a float64 array
